...
var color string //定义填充颜色

func main() {
	fmt.Printf("<svg xmlns='http://www.w3.org/2000/svg' "+ //xmlns:命名空间
		"style='stroke:grey;troke-width:0.7' "+ //stroke：线条颜色；fill：填充色；stroke-width：线条宽度
	...
	for i := 0; i < cells; i++ {
		for j := 0; j < cells; j++ {
			...
			fmt.Printf("<polygon points='%g,%g %g,%g, %g,%g, %g,%g' style='fill:%s'/>\n",
				ax, ay, bx, by, cx, cy, dx, dy, color) //多边形顶点
		}
	}
	fmt.Println("</svg>")
}
